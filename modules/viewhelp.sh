	...
	for x in $arr; do
		...
		"update")
			...
			if [[ "$updatestat" = "uptodate" ]]; then
				...
				dialog_info --timeout="5" --title="Fedora Utils is up-to-date" --text="Fedora Utils is already the latest stable version." --button="Back:0"
			elif [[ ! "$updatestat" = "available" ]]; then
				show_warn "Could not check update"
				dialog_error --title="Could not check update" --text="An error occured and Fedora Utils was unable to check for update." --button="Back:0"
			fi;;
		"repo")
		...
		"changelog")
			...
			if [[ -f changelog.txt ]]; then
				dialog_show --text-info --width=600 --height=600 --title="Changelog" --filename="changelog.txt" --button="Back:0"
				rm -f changelog.txt
			else
		...
		"logfile")
			if [[ -f "$logfile" ]]; then
				dialog_show --text-info --width=600 --height=600 --title="Logfile" --filename="$logfile" --button="Back:0"
			else
