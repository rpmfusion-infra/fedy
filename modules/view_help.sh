...
view_help() {
while shell=$(show_dialog --list --radiolist --width=300 --height=300 --title="$program Help" --text="$program $version © Satyajit Sahoo\nsatya164.github.io/$unixname" --no-headers --hide-column="2" --print-column="2" --column "Select:CHK" --column "Command" --column "Options" TRUE "update" "Check update" FALSE "repo" "Install repofile" FALSE "issue" "Report an issue" FALSE "wiki" "Visit the wiki" FALSE "changelog" "View changelog" FALSE "logfile" "View logfile" --button="Back:1" --button="Select:0"); do
    arr=$(echo $shell | tr "|" "\n")
    for x in $arr; do
        case $x in
        "update")
            ...
            if [[ "$updatestat" = "uptodate" ]]; then
                show_status "$program is up-to-date"
                show_dialog --timeout="5" --title="$program is up-to-date" --text="$program is already the latest stable version." --button="Back:0"
        ...
        "logfile")
            if [[ -f "$logfile" ]]; then
                ...
                show_err "No logfile exists. Try running $unixname with logging enabled. Use --help for more details."
            fi;;
        esac
    done
done
