...
while blocksites=$(dialog_show --list --radiolist --width=300 --height=300 --title="Block/unblock websites" --text="Block or unblock a website for all users.\nIt modifies the \"/etc/hosts\" file." --no-headers --hide-column="2" --print-column="2" --column "Select:CHK" --column "Command" --column "Option" TRUE "block" "Block a website" FALSE "unblock" "Unblock a website" FALSE "view" "View blocked websites" --button="Back:1" --button="Select:0"); do
	...
	for x in $arr; do
		...
		"unblock")
			site=$(dialog_show --entry --title="Unblock a website" --text="Enter the website to be unblocked.\nDo not enter http:// or www." --button="Back:1" --button="Unblock:0")
		...
		"view")
			...
			dialog_show --text-info --width=400 --height=400 --title="Blocked websites" --filename="sites.txt" --button="Back:0"
			rm -f "sites.txt";;
		esac
	done
	exit_state
