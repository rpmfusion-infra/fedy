...
repos=$(zenity --list --checklist --width=850 --height=600 --title="Backup repositories" --text="Select repositories to backup." --hide-header --column "Select" --column "Name" --column "Description" --ok-label="Backup" --cancel-label="Cancel" "${repolist[@]}")
if [[ $? -eq 0 ]]; then
