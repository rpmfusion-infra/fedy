...
if [[ "$(oracle_jdk_test)" = "Installed" ]]; then
    ...
    get_file_quiet "$dlurl" "jdk.htm"
    get32=$(grep "Linux x86" "jdk.htm" | grep ".rpm" | cut -d\" -f 12 | grep -v demos | head -n 1)
    file32=${get32##*/}
    get64=$(grep "Linux x64" "jdk.htm" | grep ".rpm" | cut -d\" -f 12 | grep -v demos | head -n 1)
    file64=${get64##*/}
...
[[ "$(oracle_jdk_test)" = "Installed" ]]; exit_state
}
