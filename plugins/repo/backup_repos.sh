...
if [[ $? -eq 0 ]]; then
    ...
    backupfile=$(show_dialog --width=700 --height=600 --file-selection --file-filter="*.tgz" --save --confirm-overwrite="A file with same name already exists. Overwrite file?" --title="Save repositories backup" --filename="$homedir/$unixname-repobackup-$(date +%s).tgz" --button="Cancel:1" --button="Save:0")
    if [[ $? -eq 0 ]]; then
