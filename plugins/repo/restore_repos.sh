...
if [[ $? -eq 0 ]]; then
    ...
    build_repolist "repos"
    repos=$(show_dialog --list --checklist --width=700 --height=600 --title="Select repositories to restore" --no-headers --print-column="2" --column "Select:CHK" --column "Name" --column "Description" --button="Back:1" --button="Restore selected:0" "${repolist[@]}")
    if [[ $? -eq 0 ]]; then
